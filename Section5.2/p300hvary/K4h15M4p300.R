...
ALLresult <- foreach(run=1:REP, .combine=abind::abind, .multicombine=TRUE, .init=array(0,c(nrow=p+1,ncol=(K-1)*6+1,ndim=0))) %dopar% {
  ...
  betahat[,1:3]<-Beta.target
  betahat[,4:6]<-Beta.tl
  betahat[,7:9]<-Beta.pool
  betahat[,10:12]<-Beta.tlw
  betahat[,13:15]<-Beta.poolw
  betahat[,16:18]<-Beta.tl2
  betahat[,19]<-DD
...
for (i in 1:REP) {
  beta.target[i,,]<-ALLresult[,1:3,i]
  beta.tl[i,,]<-ALLresult[,4:6,i]
  beta.pool[i,,]<-ALLresult[,7:9,i]
  beta.tlw[i,,]<-ALLresult[,10:12,i]
  beta.poolw[i,,]<-ALLresult[,13:15,i]
  beta.tl2[i,,]<-ALLresult[,16:18,i]
  dd[i,]<-ALLresult[1:M,19,i]
}
...
colnames(table)<-c("targetK1","targetK2","targetK3","tlK1","tlK2","tlK3","tl2K1","tl2K2","tl2K3","poolK1","poolK2","poolK3","pool2K1","pool2K2","pool2K3")
print(table)




save.image("K4h15M4p300.RData")
